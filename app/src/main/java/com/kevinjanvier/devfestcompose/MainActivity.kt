package com.kevinjanvier.devfestcompose
...
fun MyApp(){
    MaterialTheme {
        FlexColumn{
            inflexible {
                MyAppBar(appBarTitle = "GDG Kampala Jetpack Compose")
            }
            expanded(flex = 1f){
                VerticalScroller {
                    Column {
...
fun MyBody(){
    Padding(padding = 16.dp) {
        Card(shape = RoundedCornerShape(size = 10.dp)) {
            val image = +imageResource(R.drawable.gdg)
            Column(modifier = Spacing(16.dp)) {
               ...
               // Text(text = "Canary",style = +themeTextStyle { body2 })
                Text(text = "Kampala Uganda", style = +themeTextStyle { body2 })
            }
        }
    }
